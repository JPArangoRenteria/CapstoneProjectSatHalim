function [accumulatedTable, validLinks] = network_path_opt(scenario, constellation, numPlanes, filename, source, target, st_types, routing_step)
    sats = constellation; % Satellites in the scenario
    ...
    accessTables = createAccessTables(sats, GS,length(constellation),numPlanes);
    ...
    switch st_types
        ...
        case 1  % Ground Station to Ground Station
            src = nodeMapping.GroundStations{source};
            tgt = nodeMapping.GroundStations{target};
...
function accessTables = createAccessTables(sats, GS,numSatellites,numPlanes)
    ...
    accessTables.GS_Sat = cell(numel(GS), numel(sats));
