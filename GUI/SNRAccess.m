function streamlined = SNRAccess(scenario,accessobj,aircraft)
    % Vars needed
    ...
    streamlined = table('Size',[vectorsize 4], 'VariableTypes', {'string', 'datetime', 'datetime', 'double'}, 'VariableNames',{'Satellite', 'Connection Start', 'Connection End', 'SNR'});

    % Start of Sim
    starttime = accessobj.Intervals.StartTime(connectedaccessrow);
    starttime.TimeZone = '';
    starttime.Format = 'd-MMMM-yyyy HH:mm:ss';
    endtime = accessobj.Intervals.EndTime(connectedaccessrow);
    endtime.TimeZone = '';
    endtime.Format = 'd-MMMM-yyyy HH:mm:ss';

    streamlined(tablerow, [1, 2, 3, 4]) = {accessobj.Intervals.Source(connectedaccessrow), starttime, endtime, accessobj.Intervals.SNR(connectedaccessrow)};
    connected = true;
        ...
        if connected % If connected
            for rownumber = connectedaccessrow:size(accessobj.Intervals.Source,1)
                ...
                if intime % If theres a sat within range at the time
                    if accessobj.Intervals.SNR(rownumber) > accessobj.Intervals.SNR(connectedaccessrow) % If better SNR
                        % Disconnect from old
                        streamlined(tablerow, 3) = {currenttime};
                        ...
                        streamlined(tablerow, [1, 2, 4]) = {accessobj.Intervals.Source(connectedaccessrow), currenttime, accessobj.Intervals.SNR(connectedaccessrow)};
                    end
                end        
            end
            ...
            for nonconnectedindex = connectedaccessrow+1:size(accessobj.Intervals.Source,1)
                ...
                if intime % If there's a sat in range at the time
                    ...
                    elseif reconnected == true % If we did reconnect, multiple sats in range
                        if accessobj.Intervals.SNR(nonconnectedindex) > accessobj.Intervals.SNR(connectedaccessrow) % If better SNR
                        % Disconnect from old
                        streamlined(tablerow, 3) = {currenttime};
    ...
    streamlined = cleanuptable(scenario, streamlined);
end
